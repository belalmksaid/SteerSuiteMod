...
/// @file SocialForcesGAIModule.cpp
/// @brief Implements the SocialForcesGAIModule plugin.
...
namespace SocialForcesGGlobals
{
...
{
	return new SocialForcesGAIModule;
}
...
void SocialForcesGAIModule::init( const SteerLib::OptionDictionary & options, SteerLib::EngineInterface * engineInfo )
{
	// gEngine = engineInfo;
...
void SocialForcesGAIModule::initializeSimulation()
{
	//
...
void SocialForcesGAIModule::finish()
{
	// nothing to do here
}

void SocialForcesGAIModule::preprocessSimulation()
{


}

void SocialForcesGAIModule::preprocessFrame(float timeStamp, float dt, unsigned int frameNumber)
{
	if ( frameNumber == 1)
	{
	...
	{
		dynamic_cast<SocialForcesGAgent *>(agents_[i])->computeNeighbors();
		dynamic_cast<SocialForcesGAgent *>(agents_[i])->computeNewVelocity(dt);
	}*/
}

void SocialForcesGAIModule::postprocessFrame(float timeStamp, float dt, unsigned int frameNumber)
{
	// do nothing for now
	int i = 0;
	i = i + i;
}
SteerLib::AgentInterface * SocialForcesGAIModule::createAgent()
{
	SocialForcesGAgent * agent = new SocialForcesGAgent;
...
void SocialForcesGAIModule::destroyAgent( SteerLib::AgentInterface * agent )
{
	/*
	...
	// SocialForcesGAgent * rvoagent = dynamic_cast<SocialForcesGAgent *>(agent);
	/*
	...
	if (agent && &agents_ && (agents_.size() > 1))
	{
		...
		agents_.erase(agents_.begin()+dynamic_cast<SocialForcesGAgent *>(agent)->id());
		delete agent;
	}
...
void SocialForcesGAIModule::cleanupSimulation()
{
	agents_.clear();
