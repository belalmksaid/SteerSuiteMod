...
/// @file SocialForcesGAgent.cpp
/// @brief Implements the SocialForcesGAgent class.
...
using namespace SocialForcesGGlobals;
using namespace SteerLib;

inline bool PointInPolygon(Point point, vector<Point> points) {
...
SocialForcesGAgent::SocialForcesGAgent()
{
	_SocialForcesGParams.sf_acceleration = sf_acceleration;
	_SocialForcesGParams.sf_personal_space_threshold = sf_personal_space_threshold;
	_SocialForcesGParams.sf_agent_repulsion_importance = sf_agent_repulsion_importance;
	_SocialForcesGParams.sf_query_radius = sf_query_radius;
	_SocialForcesGParams.sf_body_force = sf_body_force;
	_SocialForcesGParams.sf_agent_body_force = sf_agent_body_force;
	_SocialForcesGParams.sf_sliding_friction_force = sf_sliding_friction_force;
	_SocialForcesGParams.sf_agent_b = sf_agent_b;
	_SocialForcesGParams.sf_agent_a = sf_agent_a;
	_SocialForcesGParams.sf_wall_b = sf_wall_b;
	_SocialForcesGParams.sf_wall_a = sf_wall_a;
	_SocialForcesGParams.sf_max_speed = sf_max_speed;
...
SteerLib::EngineInterface * SocialForcesGAgent::getSimulationEngine()
{
	return _gEngine;
}

void SocialForcesGAgent::setParameters(Behaviour behave)
{
	this->_SocialForcesGParams.setParameters(behave);
}

void SocialForcesGAgent::disable()
{
	// DO nothing for now
...
void SocialForcesGAgent::reset(const SteerLib::AgentInitialConditions & initialConditions, SteerLib::EngineInterface * engineInfo)
{
...
// initialize the agent based on the initial conditions
	...
	Room* r = getRoom(_position);
	vector<AgentGoalInfo> plan = getEvacuationPlan(r, target);

	for (int i = 0; i < plan.size(); i++) {
		_goalQueue.push(plan[i]);
	}
	...
	_prefVelocity =
		(
			...
			/
			_SocialForcesGParams.sf_acceleration
			)
		*
		MASS;
	...
	// std::cout << "Parameter spec: " << _SocialForcesGParams << std::endl;
...
Room* SocialForcesGAgent::getRoom(Point pos) {
	SimulationEngine* engine = static_cast<SimulationEngine*>(getSimulationEngine());
	for (int i = 0;i < engine->_roomsV.size(); i++) {
		if (engine->_roomsV[i]->containsAgent(pos)) {
			return engine->_roomsV[i];
		}
	}
	return NULL;
}
...
inline void createPath(vector<AgentGoalInfo>& plan, Exit* target, Room* current, Point pos) {
	if (current != target->rooms[0]) {
		map<int, pair<double, double>> prob;
		vector<int> indices;
		double sum = 0;
		for (int i = 0; i < current->exits.size(); i++) {
			Room* r = current->exits[i]->rooms[1];
			int index = 1;
			if (current->exits[i]->rooms[1]->number.find("Outside") == string::npos) {
				if (r == current) {
					r = current->exits[i]->rooms[0];
					index = 0;
				}
				indices.push_back(i);
				double w = (index == 0 ? current->exits[i]->weight1 : current->exits[i]->weight2);
				prob.insert(pair<int, pair<double, double>>(i, pair<double, double>(sum, sum + w)));
				sum += w;
			}
		}
		srand(time(NULL));
		...
		for (int i = 0; i < indices.size(); i++) {
			if (pr > prob[indices[i]].first && pr < prob[indices[i]].second)
			{
				if ((current->exits[indices[i]]->position - target->position).length() < (pos - target->position).length()) {

					{
						SteerLib::AgentGoalInfo _goal;
						_goal.targetLocation = current->exits[indices[i]]->position;
						plan.push_back(_goal);
					}
					{
						SteerLib::AgentGoalInfo _goal;
						_goal.targetLocation = (current->exits[indices[i]]->rooms[0] == current ? current->exits[indices[i]]->rooms[1] : current->exits[indices[i]]->rooms[0])->position * 1.0;
						plan.push_back(_goal);
					}
				}
				//createPath(plan, target, (current->exits[indices[i]]->rooms[0] == current ? current->exits[indices[i]]->rooms[1] : current->exits[indices[i]]->rooms[0]), pos);
				break;
			}
		}
...
inline Exit* getClosestEscapeExit(vector<Exit*>* exits, Point pos) {
	double distance = 100000000;
	double temp = 0;
	Exit* res;
	for (int i = 0; i < exits->size(); i++) {
		if ((*exits)[i]->rooms[1]->number.find("Outside") != string::npos) {
			temp = ((*exits)[i]->position - pos).length();
			if (temp < distance) {
				distance = temp;
				res = (*exits)[i];
			}
...
vector<AgentGoalInfo> SocialForcesGAgent::getEvacuationPlan(Room* current, Point target) {
	vector<AgentGoalInfo> plan;
	SimulationEngine* engine = static_cast<SimulationEngine*>(getSimulationEngine());
	//Room* tester = engine->_rooms["5"];
	//cout << target << endl;
	//cout <<  ( insidePolygon(tester->vertices, target) ? "inside" : "not inside") << endl;
	//Room* tr = getRoom(target);
	Exit* closest = getClosestEscapeExit(&(engine->_exitsV), _position);
	createPath(plan, closest, current, _position);
	SteerLib::AgentGoalInfo _goal;
	_goal.targetLocation = closest->position * 1.0;
	plan.push_back(_goal);

	return plan;
}

Point SocialForcesGAgent::getTarget() {
	SimulationEngine* engine = static_cast<SimulationEngine*>(getSimulationEngine());
	double distance = 10000000000000.0;
	double temp = 0;
	Point point;
	int index = -1;
	for (int i = 0; i < engine->_evacTargets.size(); i++) {
		temp = (engine->_evacTargets[i].position - _position).length();
		if (temp < distance) {
			distance = temp;
			point = (engine->_evacTargets[i].position);
			index = i;
		}
	}
	if (index == -1) {
		for (int i = 0; i < engine->_roomsV.size(); i++) {
			if (engine->_roomsV[i]->number.find("Outside") != std::string::npos) {
				temp = (engine->_roomsV[i]->position - _position).length();
				if (temp < distance) {
					distance = temp;
					point = engine->_roomsV[i]->position * 1.0;
				}
			}
		}
	}
...
void SocialForcesGAgent::calcNextStep(float dt)
{
}
...
Util::Vector SocialForcesGAgent::calcProximityForce(float dt)
{
	...
	getSimulationEngine()->getSpatialDatabase()->getItemsInRange(_neighbors,
		_position.x - (this->_radius + _SocialForcesGParams.sf_query_radius),
		_position.x + (this->_radius + _SocialForcesGParams.sf_query_radius),
		_position.z - (this->_radius + _SocialForcesGParams.sf_query_radius),
		_position.z + (this->_radius + _SocialForcesGParams.sf_query_radius),
		...
		if ((*neighbour)->isAgent())
		{
			...
			away = away +
				(
					...
					*
					(
						_SocialForcesGParams.sf_agent_a
						*
						exp(
							(
								(
								...
								/
								_SocialForcesGParams.sf_agent_b
								)
							)
			...
			if (obs_cir != NULL && USE_CIRCLES)
			{
				...
				away = away +
					(
						...
						*
						(
							_SocialForcesGParams.sf_wall_a
							*
							exp(
								(
									(
									...
									/
									_SocialForcesGParams.sf_wall_b
									)
								)
...
Util::Vector SocialForcesGAgent::calcRepulsionForce(float dt)
{
#ifdef _DEBUG_
	std::cout << "wall repulsion; " << calcWallRepulsionForce(dt) << " agent repulsion " <<
		(_SocialForcesGParams.sf_agent_repulsion_importance * calcAgentRepulsionForce(dt)) << std::endl;
#endif
	return calcWallRepulsionForce(dt) + (_SocialForcesGParams.sf_agent_repulsion_importance * calcAgentRepulsionForce(dt));
}

Util::Vector SocialForcesGAgent::calcAgentRepulsionForce(float dt)
{
		...
		// for (int a =0; a < tmp_agents.size(); a++)
	{
			...
			)
		{
			agent_repulsion_force = agent_repulsion_force +
				(tmp_agent->computePenetration(this->position(), this->radius()) * _SocialForcesGParams.sf_agent_body_force * dt) *
		...
		/*
			...
			agent_repulsion_force = agent_repulsion_force +
				(_SocialForcesGParams.sf_sliding_friction_force * dt *
					(
...
Util::Vector SocialForcesGAgent::calcWallRepulsionForce(float dt)
{
					...
					((
						(
							...
							*
							(
								radius() +
								_SocialForcesGParams.sf_personal_space_threshold -
								(
									distance
									)
								)
						...
						/
						distance
						)* _SocialForcesGParams.sf_body_force * dt);
...
std::pair<Util::Point, Util::Point> SocialForcesGAgent::calcWallPointsFromNormal(SteerLib::ObstacleInterface* obs, Util::Vector normal)
{
 ...
 *
 */
Util::Vector SocialForcesGAgent::calcWallNormal(SteerLib::ObstacleInterface* obs)
{
	Util::AxisAlignedBox box = obs->getBounds();
 ...
 */
Util::Vector SocialForcesGAgent::calcObsNormal(SteerLib::ObstacleInterface* obs)
{
	Util::AxisAlignedBox box = obs->getBounds();
...
/*
void SocialForcesGAgent::computeNeighbors()
{
	agentNeighbors_.clear();

	if (_SocialForcesGParams.rvo_max_neighbors > 0) {
		// std::cout << "About to segfault" << std::endl;
		dynamic_cast<SocialForcesGAIModule *>(rvoModule)->kdTree_->computeAgentNeighbors(this, _SocialForcesGParams.rvo_neighbor_distance * _SocialForcesGParams.rvo_neighbor_distance);
		// std::cout << "Made it past segfault" << std::endl;
	}
}*/


void SocialForcesGAgent::updateAI(float timeStamp, float dt, unsigned int frameNumber)
{
	// std::cout << "_SocialForcesGParams.rvo_max_speed " << _SocialForcesGParams._SocialForcesGParams.rvo_max_speed << std::endl;
	Util::AutomaticFunctionProfiler profileThisFunction(&SocialForcesGGlobals::gPhaseProfilers->aiProfiler);
	if (!enabled())
	{
		return;
	}
	...
	Util::Vector prefForce = (((goalDirection * PERFERED_SPEED) - velocity()) / (_SocialForcesGParams.sf_acceleration / dt)); //assumption here
	...
	if (repulsionForce.x != repulsionForce.x)
	{
		...
		// throw GenericException("SocialForcesG numerical issue");
	}
	...
	_velocity = clamp(velocity(), _SocialForcesGParams.sf_max_speed);
	_velocity.y = 0.0f;
#ifdef _DEBUG_
	...
	/*
		...
		*/
		/*
		 * Now do the conversion from SocialForcesGAgent into the SteerSuite coordinates
		 */
		 // _velocity.y = 0.0f;
