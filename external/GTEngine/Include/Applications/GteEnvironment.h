...
public:
    ...
    static bool FullPath(std::string const& relativePath, std::string& fullPath);
    ...
    std::string GetPath(std::string const& name, FileMode mode = FM_READABLE) const;

    // String formatting operations.
    static std::string CreateString(char const* format, ...);
    static std::string CreateStringFromArgs(char const* format, va_list arguments);
    static std::wstring CreateString(wchar_t const* format, ...);
    static std::wstring CreateStringFromArgs(wchar_t const* format, va_list arguments);
