...
void TestCasePlayerModule::initializeSimulation() {
	...
	for (unsigned int i = 0;i < testCaseReader->getNumRooms(); i++) {
		_engine->addRoom(testCaseReader->getRoom(i));
	}
	for (unsigned int i = 0;i < testCaseReader->getNumExits(); i++) {
		_engine->addExit(testCaseReader->getExit(i));
	}
	for (unsigned int i = 0;i < testCaseReader->getNumEvacTargets(); i++) {
		_engine->addEvacuationTarget(testCaseReader->getEvacTarget(i));
	}
