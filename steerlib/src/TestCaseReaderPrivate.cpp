...
void TestCaseReaderPrivate::_parseTestCaseDOM(const ticpp::Element * root)
{
	...
	for (child = child.begin(root); child != child.end(); child++) {
		...
		if (childTagName == "header") {
			...
			else if (childTagName == "RoomsExits") {
				_parseRoomsExits(&(*child));
			}
			else if (childTagName == "EvacuationTargets") {
				_parseEvacuationTargets(&(*child));
			}
			else {
				throw GenericException("Unexpected tag <" + childTagName + "> found on line " + toString(child->Row()) + "\n");
			}
...
void TestCaseReaderPrivate::_parseRoomsExits(const ticpp::Element * subRoot)
{
	ticpp::Iterator<ticpp::Element> child;
	for (child = child.begin(subRoot); child != child.end(); child++) {
		std::string childTagName = child->Value();
		if (childTagName == "Room") {
			Room* room = new Room();
			_getRoomFromXMLElement(&(*child), room);
			_rooms.insert(std::pair<std::string, Room*>(room->number, room));
			_roomsV.push_back(room);
		}
		else if (childTagName == "Exit") {
			Exit* exit = new Exit();
			_getExitFromXMLElement(&(*child), exit);
			_exits.insert(std::pair<std::string, Exit*>(exit->number, exit));
			_exitsV.push_back(exit);
		}
		else {
			throw GenericException("Unexpected tag <" + childTagName + "> found on line " + toString(child->Row()) + "\n");
		}
...
void TestCaseReaderPrivate::_getRoomFromXMLElement(const ticpp::Element * subRoot, Room * room) {
	ticpp::Iterator<ticpp::Element> child;
	for (child = child.begin(subRoot); child != child.end(); child++) {
		std::string childTagName = child->Value();
		if (childTagName == "Location") {
			Point v;
			bool isRand;
			_getXYZOrRandomFromXMLElement(&(*child), v, isRand);
			room->position = v;
		}
		else if (childTagName == "Vertex") {
			Point v;
			bool isRand;
			_getXYZOrRandomFromXMLElement(&(*child), v, isRand);
			room->vertices.push_back(v);
		}
		else if (childTagName == "Number") {
			child->GetText(&(room->number));
		}
		else if (childTagName == "Area") {
			child->GetText(&(room->area));
		}
		else {
			throw GenericException("Unexpected tag <" + childTagName + "> found on line " + toString(child->Row()) + "\n");
		}
...
void TestCaseReaderPrivate::_getExitFromXMLElement(const ticpp::Element * subRoot, Exit * exit) {
	ticpp::Iterator<ticpp::Element> child;
	for (child = child.begin(subRoot); child != child.end(); child++) {
		std::string childTagName = child->Value();
		if (childTagName == "Location") {
			...
			child->GetText(&(exit->number));
		}
		else if (childTagName == "Room") {
			Room* room;
			_getRoomForExitFromXMLElement(&(*child), exit, &room);
			exit->rooms.push_back(room);
...
void TestCaseReaderPrivate::_getRoomForExitFromXMLElement(const ticpp::Element* subRoot, Exit * exit, Room** room) {
	ticpp::Iterator<ticpp::Element> child;
	for (child = child.begin(subRoot); child != child.end(); child++) {
		std::string childTagName = child->Value();
		if (childTagName == "Number") {
			string num;
			child->GetText(&(num));
			*room = _rooms[num];
		}
		else if (childTagName == "Weight") {
			if (exit->weight1 < 0) {
				child->GetText(&(exit->weight1));
			}
			else
			{
				child->GetText(&(exit->weight2));
			}
		}
		else {
			throw GenericException("Unexpected tag <" + childTagName + "> found on line " + toString(child->Row()) + "\n");
...
void TestCaseReaderPrivate::_parseEvacuationTargets(const ticpp::Element * subRoot) {
	ticpp::Iterator<ticpp::Element> child;
	for (child = child.begin(subRoot); child != child.end(); child++) {
		std::string childTagName = child->Value();
		if (childTagName == "EvacuationTarget") {
			EvacuationTarget et;
			_getEvacuationTargetFromXML(&(*child), et);
			_evacTargets.push_back(et);
		}
		else {
			throw GenericException("Unexpected tag <" + childTagName + "> found on line " + toString(child->Row()) + "\n");
		}
...
void TestCaseReaderPrivate::_getEvacuationTargetFromXML(const ticpp::Element* subRoot, EvacuationTarget& evacuationTarget) {
	ticpp::Iterator<ticpp::Element> child;
	for (child = child.begin(subRoot); child != child.end(); child++) {
		std::string childTagName = child->Value();
		if (childTagName == "Location") {
			Point v;
			bool isRandom;
			_getXYZOrRandomFromXMLElement(&(*child), v, isRandom);
			evacuationTarget.position = v;
		}
		else {
			throw GenericException("Unexpected tag <" + childTagName + "> found on line " + toString(child->Row()) + "\n");
		}
