...
void SimulationEngine::addRoom(SteerLib::Room* room) {
	_roomsV.push_back(room);
	_rooms.insert(std::pair<std::string, Room*>(room->number, room));
}
void SimulationEngine::addExit(SteerLib::Exit* exit) {
	_exitsV.push_back(exit);
	_exits.insert(std::pair<std::string, Exit*>(exit->number, exit));
}
void SimulationEngine::addEvacuationTarget(SteerLib::EvacuationTarget evacTarget) {
	_evacTargets.push_back(evacTarget);
}
