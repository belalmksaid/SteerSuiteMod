...
namespace SteerLib {
	...
	protected:
		...
		/// Reads Exit
		void _getExitFromXMLElement(const ticpp::Element * subRoot, Exit * exit);
		/// Reads Room
		void _getRoomFromXMLElement(const ticpp::Element * subRoot, Room * room);
		/// Read RoomsExits
		void _parseRoomsExits(const ticpp::Element * subRoot);
		/// Read subrooms for exits
		void _getRoomForExitFromXMLElement(const ticpp::Element* subRoot, Exit * exit, Room** room);
		/// Get evacuation targets
		void _parseEvacuationTargets(const ticpp::Element* subRoot);
		///
		void _getEvacuationTargetFromXML(const ticpp::Element* subRoot, EvacuationTarget& evacuationTarget);
		//@}
		...
		/// Room and Exits Graph
		std::map<std::string, Room*> _rooms;
		std::vector<Room*> _roomsV;
		std::map<std::string, Exit*> _exits;
		std::vector<Exit*> _exitsV;
		std::vector<EvacuationTarget> _evacTargets;
	};
