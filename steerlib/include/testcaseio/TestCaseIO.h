...
namespace SteerLib {
	...
	public:
		...
		inline size_t getNumRooms() const { return _rooms.size(); }
		/// Returns number of exits
		inline size_t getNumExits() const { return _exits.size(); }
		/// Returns number of rooms
		inline size_t getNumEvacTargets() const { return _evacTargets.size(); }
		...
		/// Returns Rooms Pair
		inline Room* getRoom(unsigned int index) const { return _roomsV.at(index); }
		inline Exit* getExit(unsigned int index) const { return _exitsV.at(index); }
		inline EvacuationTarget getEvacTarget(unsigned int index) const { return _evacTargets.at(index); }

		inline void copyRoomMap(std::map<string, Room*> * mapt) {
			mapt->insert(_rooms.begin(), _rooms.end());
		}
		inline void copyExitMap(std::map<string, Exit*> * mapt) {
			mapt->insert(_exits.begin(), _exits.end());
		}
		//@}
	};
