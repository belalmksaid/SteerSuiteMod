...
namespace SteerLib {
	class STEERLIB_API SimulationEngine;
	/**
	...
	public:
		...
		/// @name engine wrapping functions
		/// @brief These functions are used to wrap the steersuite engine so that the entire simulation can be put into a function. These member functions help start, get data from and terminate the engine.
		//@{
		virtual void init(SteerLib::SimulationOptions * options) = 0;
