...
void PredictiveAvoidanceAIModule::preprocessFrame(float timeStamp, float dt, unsigned int frameNumber)
{
	...
	{
		dynamic_cast<PredictiveAvoidanceAgent *>(agents_[i])->computeNeighbors();
		dynamic_cast<PredictiveAvoidanceAgent *>(agents_[i])->computeNewVelocity(dt);
	}*/
}
...
void PredictiveAvoidanceAIModule::destroyAgent( SteerLib::AgentInterface * agent )
{
	...
	// PredictiveAvoidanceAgent * rvoagent = dynamic_cast<PredictiveAvoidanceAgent *>(agent);
	/*
	...
	if (agent && &agents_ && (agents_.size() > 1))
	{
		...
		agents_.erase(agents_.begin()+dynamic_cast<PredictiveAvoidanceAgent *>(agent)->id());
		delete agent;
	}
