...
OracleEnvironment::OracleEnvironment( OracleAgent * newAgent )
{ 
	_numNodesExpanded = 0;
...
OracleEnvironment::~OracleEnvironment() {
	/*cout << _numNodesExpanded << endl;
	cout.flush();*/
}


float OracleEnvironment::getHeuristic(int start, int target) const
{
...
inline bool OracleEnvironment::_addFootstepIfValid(float stepDuration, float parabolaOrientationPhi, bool phiIsIdeal, float desiredVelocity, const Footstep & previousStep, FootStateEnum nextState, vector<Successor> & result) const
...
void OracleEnvironment::getSuccessors(int nodeId, int lastNodeId, vector<Successor> & result) const
{
	result.clear();
...
*/
	...
	if ((previousStep.state == FOOTSTEP_STATE_NORMAL) || (previousStep.state == FOOTSTEP_STATE_STARTING)) {
		...
		if (previousStep.simulationA < 0.9f) {
			...
			/* The next three options were originally commented out - CORY */
			_addFootstepIfValid(0.6f, parabolaPhiStraight + 0.05f*radiansInterval, false, 1.3f, previousStep, FOOTSTEP_STATE_NORMAL, result);
			_addFootstepIfValid(0.6f, parabolaPhiStraight + 0.075f*radiansInterval, false, 1.3f, previousStep, FOOTSTEP_STATE_NORMAL, result);
			_addFootstepIfValid(0.6f, parabolaPhiStraight + 0.1f*radiansInterval, false, 1.3f, previousStep, FOOTSTEP_STATE_NORMAL, result);
			...
			/* The following two choices were originally commented out - CORY */
			_addFootstepIfValid(0.6f, parabolaPhiStraight + 0.05f*radiansInterval, false, 0.5f, previousStep, FOOTSTEP_STATE_NORMAL, result);
			_addFootstepIfValid(0.6f, parabolaPhiStraight + 0.075f*radiansInterval, false, 0.5f, previousStep, FOOTSTEP_STATE_NORMAL, result);
	...
	else if (previousStep.state == FOOTSTEP_STATE_STARTING) { /* This else-if block was originally commented out - CORY*/
		// valid state transitions are:
		...
		_addFootstepIfValid(0.4f, idealParabolaPhi, 0.5f, true, previousStep, FOOTSTEP_STATE_NORMAL, result);
...
bool OracleEnvironment::isValidNodeId(int nodeId) const
{
