...
#ifndef __FOOT_REC_IO_H__
#define __FOOT_REC_IO_H__
...
//
// TODO: Fill in the new transitions
// 
// new transitions:
// ??? -> HOPPING (whichFoot alternates???)
// HOPPING -> ??? (whichFoot alternates???)
// STATIONARY -> BACKSTEP (whichFoot can be either)
// INPLACETURNING
...
struct Footstep {
	Footstep()
	{
		simulationA = simulationDx = simulationIx = simulationIz = simulationJx = simulationJz = 0.0f;
	}
...
class FootRecWriter {
public:
	FootRecWriter(size_t numAgents, size_t numObstacles);
	~FootRecWriter();
	...
	/// Writes to a 3ds max STP file, specifying footsteps for a biped.
	void writeToStepFile(const std::string & filename);
...
class FootRecReader {
public:
	explicit FootRecReader(const std::string & filename);
	~FootRecReader();
