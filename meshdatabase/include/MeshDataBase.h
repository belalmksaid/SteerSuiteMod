...
#ifndef MESHDATABASE_H_
#define MESHDATABASE_H_
...
namespace SteerLib
{
	...
	class STEERLIB_API MeshDataBase : public SteerLib::SpatialDataBaseInterface
	{
	public:
		MeshDataBase(ACCLMesh::ACCLMesh * mesh, EngineInterface * gEngine);
		...
		virtual Util::Point getLocation(SpatialDatabaseItemPtr exclude1);
		/// Get Normal for item, This get the normal for an item, used to determine the orientation of the item
		virtual Util::Vector getUpVector(SpatialDatabaseItemPtr exclude1);

		virtual Util::Point closestVert(Util::Point);
	...
	protected:
		...
		void setMesh(ACCLMesh::ACCLMesh * mesh)
		{
			this->_mesh = mesh;
		}

		virtual std::vector<size_t> getFacesWithin(double dist);

		std::vector<std::vector<size_t> > _obstacels;

	public:
		friend class NavMeshModule;
		MeshDataBase * _spatialDatabase; // Pointer to itself??
		KdTreeDataBase * _kdTreeDatabase;
		// face occupency list as in agent in each face
		std::vector<std::set<size_t>> _face_occupency;
		// agent occupency list, as in the face each agent is in
		std::vector<size_t> _agent_face_map;

	private:
		ACCLMesh::ACCLMesh * _mesh;
	};
}


#endif /* NAVMESHDATABASE_H_ */
